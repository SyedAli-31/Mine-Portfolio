...
const WEB3FORMS_KEY = "b0479c74-c682-4bca-b0df-cca21cc766e9"; // 🔑 your key
...
const Contact = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      formData.append("to_email", WEB3FORMS_EMAIL); // 👈 Force Gmail delivery
  ...
  return (
    <section id="contact" className="relative" ref={ref}>
      ...
      <div className="absolute inset-0 z-0 overflow-hidden opacity-10">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" className="text-gray-300">
          <path
            fill="currentColor"
            fillOpacity="1"
        ...
        >
          {isClient && (
            <motion.div
              ...
              className="bg-[hsl(222,84%,5%)] backdrop-blur-sm rounded-xl p-8 border border-gray-800"
            >
                              ...
                              <FormControl>
                                {fieldName === "message" ? (
                                  <Textarea
                                    ...
                                    className="bg-gray-200 border-gray-700 text-gray-700 focus:border-primary focus:ring-primary"
                                    rows={5}
                                    {...field}
                                  />
                                ) : (
          ...
          {isClient && (
            <motion.div
              variants={itemVariants}
              className="bg-[hsl(222,84%,5%)] backdrop-blur-sm rounded-xl p-8 border border-gray-800 h-full"
            >
              <h3 className="text-2xl font-semibold mb-6 text-white">Connect With Us</h3>
              <div className="space-y-8 mb-8">
                <ContactInfo icon={MapPin} title="Our Location" text="Pakistan, Karachi" />
                <ContactInfo icon={Mail} title="Email Us" text={WEB3FORMS_EMAIL} />
                <ContactInfo icon={Phone} title="Call Us" text="+92 3486906754" subtext="Mon-Fri, 9AM-6PM PST" />
              </div>
            </motion.div>
          )}
        </motion.div>
